...
import { Wallet, ArrowDownRight, ArrowUpRight, CheckCircle2, Clock, AlertCircle, Phone, CreditCard, Building2, ShieldCheck } from 'lucide-react';
...
export const WalletPage: React.FC<WalletPageProps> = ({
  ...
  return (
    <div className="space-y-5 pb-24">
      {/* Header */}
      <div>
        <h2 className="text-2xl font-black text-white">Points & Rewards Wallet</h2>
        <p className="text-xs text-slate-400">Track your earnings and cash out to Telebirr / CBE Birr</p>
      </div>
      ...
      <div className="p-6 rounded-3xl bg-[#0f0f15] border border-white/10 space-y-4 shadow-xl relative overflow-hidden">
        ...
        <div className="flex items-center justify-between">
          <span className="text-xs text-slate-400 font-medium uppercase tracking-wider">Available Cash Balance</span>
        ...
        <div>
          <h2 className="text-4xl font-black text-transparent bg-clip-text bg-gradient-to-r from-indigo-400 via-purple-300 to-purple-500">
            {user.balance.toLocaleString()} <span className="text-amber-400 text-2xl font-bold">Birr</span>
          </h2>
          <p className="text-xs text-indigo-400 mt-1">Equivalent Points: {user.points.toLocaleString()} PTS</p>
        </div>

        {/* Progress Bar */}
        <div className="space-y-1.5 pt-3 border-t border-white/10">
          <div className="flex items-center justify-between text-xs">
            <span className="text-slate-400">Cashout Threshold (2,000 Birr Min)</span>
        ...
        >
          ...
          <span>{isEligible ? 'Request Cash Out Now' : `Earn ${(2000 - user.balance).toLocaleString()} More Birr To Cash Out`}</span>
        </button>
      </div>

      {/* Payment Partners Showcase */}
      <div className="p-5 rounded-3xl bg-[#0f0f15] border border-white/5 space-y-3">
        <h4 className="text-xs font-bold text-slate-400 uppercase tracking-wider">Official Cashout Payment Partners</h4>
        <div className="grid grid-cols-3 gap-2 text-center text-xs">
          ...
          <div className="p-3 rounded-2xl bg-white/5 border border-white/5 space-y-1">
            ...
            <span className="text-[9px] text-slate-400 block">Commercial Bank</span>
          </div>
          <div className="p-3 rounded-2xl bg-white/5 border border-white/5 space-y-1">
            ...
            <span className="text-[9px] text-slate-400 block">All Ethio Banks</span>
          </div>
        </div>
      </div>
        ...
        <div className="space-y-2">
          ...
          {filteredTxs.length === 0 && (
            <div className="p-8 rounded-3xl bg-[#0f0f15] border border-white/5 text-center text-slate-400 text-xs">
              No transactions logged yet. Complete tasks or check in daily to start earning!
            </div>
          )}
        </div>
      </div>
